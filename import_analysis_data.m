...
while (true)
    ...
    if (type == '1')
        ...
        [analysis_data.vesicle(m).name,...
         ...
         analysis_data.vesicle(m).r,...
         analysis_data.vesicle(m).minor,...
         analysis_data.vesicle(m).major,...
         analysis_data.vesicle(m).areapx,...
         analysis_data.vesicle(m).ar] = strread(records, '%s %f %f %f %f %f %f %f');
    ...
    elseif (type == '3')%this is the freehand selection tool in imageJ to annotate closed objects with irregular shape
        ...
        [freeh(k).name,...
         ...
         freeh(k).area,...
         freeh(k).xcoords,...
         freeh(k).ycoords] = strread(records, '%s %f %f %s %s');
        ...
        if strcmp(freeh(k).name, 'Endosome')
            ...
            analysis_data.endosome(t_4).length = freeh(k).length*pixel_size;
            analysis_data.endosome(t_4).area = freeh(k).area*pixel_size^2;
            
            
            analysis_data.endosome(t_4).x    = strread(freeh(k).xcoords{1},...
                              '%f','delimiter',',');
            
            analysis_data.endosome(t_4).y    = strread(freeh(k).ycoords{1},...
                              '%f','delimiter',',');
                          
            t_4 = t_4+1;

        elseif strcmp(freeh(k).name, 'Plasma_membrane')
            
            analysis_data.pm(c).name = char(freeh(k).name);
            
            analysis_data.pm(c).length = freeh(k).length*pixel_size;

            analysis_data.pm(c).area = freeh(k).area*pixel_size^2;
            
            analysis_data.pm(c).x    = strread(freeh(k).xcoords{1},...
                                 '%f','delimiter',',');
            
            analysis_data.pm(c).y    = strread(freeh(k).ycoords{1},...
                                 '%f','delimiter',',');
        ...
        elseif strcmp(freeh(k).name, 'fEndosome')
            ...
            analysis_data.fendosome(t_5).x    = strread(freeh(k_1).xcoords{1},...
                              '%f','delimiter',',');
            
            analysis_data.fendosome(t_5).y    = strread(freeh(k_1).ycoords{1},...
                              '%f','delimiter',',');
        ...
        elseif strcmp(freeh(k).name, 'MVB')
            ...
            analysis_data.mvb(t_6).x    = strread(freeh(k).xcoords{1},...
                              '%f','delimiter',',');
            
            analysis_data.mvb(t_6).y    = strread(freeh(k).ycoords{1},...
                              '%f','delimiter',',');
        ...
        elseif strcmp(freeh(k).name, 'fMVB')
            ...
            analysis_data.fmvb(t_7).x    = strread(freeh(k_1).xcoords{1},...
                              '%f','delimiter',',');
            
            analysis_data.fmvb(t_7).y    = strread(freeh(k_1).ycoords{1},...
                              '%f','delimiter',',');
