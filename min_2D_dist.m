...
if isfield (analysis_data, 'dp')
    temp_min = Inf;
    closest_dp = 0; %initializes index for closer dp

for j=1:length(analysis_data.vesicle)
    temp_min = Inf;
    closest_dp = 0;
    for h = 1:length(analysis_data.dp)
    
    for i = 1:length(analysis_data.dp(h).x)
        
        temp_distance(1, i) = dist2(analysis_data.vesicle(j).x,...
                                    analysis_data.vesicle(j).y,...
                                    analysis_data.vesicle(j).r,...
                                    analysis_data.dp(h).x(i),...
                                    analysis_data.dp(h).y(i));
    end
    ...
    % if it's docked_sv then distance should be 0.
        ...
        % applying pixel size to the distance, and rounds up the value
    distance_data.vesicle(j).dist2dp = temp_min;
    distance_data.vesicle(j).closest_dp = closest_dp;
    clear temp_min;
...
%             pt(4) = floor(pt(2)*3/4);
    ...
    for i = 1:length(analysis_data.endosome)
        ...
        for j = 1:length(analysis_data.dp(h).x)
            
            for k = 1:length(analysis_data.endosome(i).x)
            
                temp_distance_1(h,a) = dist2(analysis_data.endosome(i).x(k),...
                                             analysis_data.endosome(i).y(k),...
                                             0,...
                                             analysis_data.dp(h).x(j),...
                                             analysis_data.dp(h).y(j));
            
                a=a+1;
            end
        end
        end
        distance_data.endosome(i).dist2dp = floor(min(temp_distance_1(1:h,:))*pixel_size);
